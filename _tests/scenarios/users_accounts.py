...
class UsersAccounts:
	...
	def add_first_user(self):
		...
		#Get Aaron friends
		print self.api_facade.request_user_friends(bt_token)
	...
	def add_first_three_friends(self):
		...
		# Get Aaron account.
		aaron_account_info = self.api_facade.request_user_me(aaron_bt_access_token)
		assert aaron_account_info['bt_name'] == 'Aaron Chase' 
		assert len(aaron_account_info['friends']) == 0
		# Request Aaron friends. There should be none.
		aaron_friends = self.api_facade.request_user_friends(aaron_bt_access_token)
		...
		# Get Steve account.
		steve_account_info = self.api_facade.request_user_me(steve_bt_access_token)
		...
		steve_friends = self.api_facade.request_user_friends(steve_bt_access_token)
		...
		# Get Loic account.
		loic_account_info = self.api_facade.request_user_me(loic_bt_access_token)
		...
		loic_friends = self.api_facade.request_user_friends(loic_bt_access_token)
		...
		print self.api_facade.request_user_friends(steve_bt_access_token)
		# Check friends via /user/friends api
		steve_friends = self.api_facade.request_user_friends(steve_bt_access_token)
